...
your own laptop, switch `.env`:

```
DATABASE_URL=postgresql://user:password@localhost:5432/ledger
```

`psycopg2-binary` is already in `requirements.txt`, so no extra install is
needed. Once you're on Postgres for real, replace the `Base.metadata.create_all`
call in `app/main.py` with proper Alembic migrations — `create_all` is fine
for getting started but won't handle schema changes safely later.
